...
void printUsage() {
  std::cout
      << "Usage:\n"
      << "  tdl_face_attribute_demo --image FILE --model-spec FILE\n"
...
int main(int argc, char **argv) {
  ...
  if (!classifier.load(image_demo_support::faceAttributeConfig(opt.common),
                       &error)) {
  ...
  const bool ok = opt.has_roi
                      ? classifier.run(opt.common.image, opt.roi, infer_options,
                                       &result, &error)
                      : classifier.run(opt.common.image, infer_options, &result,
                                       &error);
  if (!ok) {
