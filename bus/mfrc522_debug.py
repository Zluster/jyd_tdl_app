...
from sensor_uart import (SENSOR_TYPE_MFRC522,
                         SENSOR_UART_COMMAND_QUERY_SENSOR,
                         SENSOR_UART_COMMAND_SCAN,
                         SensorData)
...
def print_card(data: SensorData) -> None:
    """打印一帧 MFRC522 数据。"""
...
def scan_mfrc522(api: SensorApi) -> list[int]:
    ...
    nodes = sorted({step.sensor_number for step in result.steps
                    if step.sensor_type == SENSOR_TYPE_MFRC522})
    if nodes:
...
def query_once(api: SensorApi, sensor_number: int) -> SensorData:
    """主动查询指定节点并返回最新数据。"""
    result = api.command(SENSOR_UART_COMMAND_QUERY_SENSOR,
                         SENSOR_TYPE_MFRC522, sensor_number)
    if result.status != 0:
...
def listen(api: SensorApi, sensor_number: int, interval_ms: int) -> None:
    """开启自动上报并持续打印指定节点的新数据。"""
    api.set_auto_upload(SENSOR_TYPE_MFRC522, sensor_number, True, interval_ms)
    ...
    try:
        ...
        while True:
            try:
                data = api.read(SENSOR_TYPE_MFRC522, sensor_number)
        ...
        api.set_auto_upload(SENSOR_TYPE_MFRC522, sensor_number, False, 0)
