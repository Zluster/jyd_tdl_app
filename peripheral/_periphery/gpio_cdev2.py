...
try:
    KERNEL_VERSION = tuple([int(s) for s in os.uname().release.split(".")[:2]]) # DARA: os.uname().release is faster
except ValueError:
    KERNEL_VERSION = (0, 0)
...
class Cdev2GPIO(GPIO):
    ...
    def _reopen(self, direction, edge, bias, drive, inverted):
        ...
        if direction == "in":
            ...
            # DARA: _GPIO_V2_LINE_FLAG_EVENT_CLOCK_REALTIME is not supported on linux 5.10
            # https://github.com/vsergeev/python-periphery/issues/70
