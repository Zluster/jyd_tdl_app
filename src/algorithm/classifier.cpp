...
EngineConfig toEngineConfig(const Classifier::Config &config) {
  EngineConfig out;
...
std::string resolveModelType(const Classifier::Config &config) {
...
bool Classifier::load(const Config &config, std::string *error) {
  ...
  model_.reset(new NnClassifier(requested_model_type_));
