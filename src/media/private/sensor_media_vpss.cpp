...
bool SensorMediaImpl::startVpss(std::string *error) {
  ...
  const int pipe = 0;
  for (std::size_t index = 0; index < vpss_outputs_.size(); ++index) {
    auto &output = vpss_outputs_[index];
    int ret = CVI_SUCCESS;
    bool group_already_created = false;
    bool group_already_started = false;
    bool group_already_bound = false;
    for (std::size_t prev = 0; prev < index; ++prev) {
      if (vpss_outputs_[prev].config.group != output.config.group) {
        continue;
      }
      group_already_created = group_already_created || vpss_outputs_[prev].created;
      group_already_started = group_already_started || vpss_outputs_[prev].started;
      group_already_bound = group_already_bound || vpss_outputs_[prev].vi_bound;
    }

    if (!group_already_created) {
    ...
    if (output.config.bind_from_vi && !group_already_bound) {
      MMF_CHN_S src;
      MMF_CHN_S dst;
      ...
      // CVITEK samples bind VI to the VPSS group entry, not to an output
      // channel. Individual VPSS output channels are configured separately.
      dst.s32ChnId = 0;
